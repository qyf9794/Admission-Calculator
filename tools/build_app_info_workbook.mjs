...
const profileRows = [
  ...
  ["选校", "自动推荐保底数量", "requestedLikelyCount", "0-10", "必填", "学生输入", "推荐组合", "点击自动推荐按钮后按保底档填充学校"],
  ["选校", "自动推荐目标数量", "requestedTargetCount", "0-12", "必填", "学生输入", "推荐组合", "点击自动推荐按钮后按目标档填充学校"],
  ["选校", "自动推荐争取数量", "requestedReachCount", "0-12", "必填", "学生输入", "推荐组合", "点击自动推荐按钮后按争取档填充学校"],
...
const probabilityRows = [
  ...
  ["结果分档", "bucket", "0% 硬门槛未满足；>0 且 <20% 争取；20%-60% 目标；>=60% 保底", "中文标签", "结果展示、自动推荐", "计算引擎", "分档不是录取结论；保底也不是保证；自动推荐按用户指定数量从三档抽取"],
  ...
  ["推荐缺口", "recommendationWarnings", "自动推荐无法满足某档请求数量时生成提示", "列表", "结果展示、报告", "计算引擎", "某档不足时不跨档硬凑，必须披露"],
  ["T10至少一所", "t10AtLeastOne", "当前组合内 T10 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "只统计用户手选或自动推荐组合"],
  ["T30至少一所", "t30AtLeastOne", "当前组合内 T30 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "不是全库 T30，也不是简单独立相乘"],
  ["T50至少一所", "t50AtLeastOne", "当前组合内 T50 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "当前组合外学校不参与"],
