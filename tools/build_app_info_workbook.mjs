...
const profileRows = [
  ...
  ["基础", "AP 平均分", "apAverageScore", "1.0-5.0", "AP 门数 > 0 时显示", "学生输入", "课程体系成绩评分", "AP 门数为 0 时不计入课程体系成绩"],
  ...
  ["基础", "申请轮次", "round", "EA / ED / RD", "必填", "学生输入", "申请策略修正、轮次门槛", "学校级轮次数据会区分允许轮次和明确概率优势；仅允许早申不等于加分，UC 等限制走 gate"],
  ["基础", "是否申请资助", "needsAid", "是/否", "必填", "学生输入", "申请策略修正", "need-aware 学校可能降低概率"],
  ["标化", "是否不提交标化", "testOptional", "是/否", "必填", "学生输入", "硬门槛、标化评分", "test-required 学校未提交时直接 0%；打开后清空/忽略 SAT/ACT 残留值"],
  ["标化", "SAT", "sat", "900-1600", "条件必填", "学生输入", "硬门槛、标化评分", "required 学校会检查最低等效分"],
  ["标化", "ACT", "act", "18-36", "可选", "学生输入", "硬门槛、标化评分", "按 ACT/College Board 2018 concordance midpoint 换算为 SAT 等效值"],
  ["语言", "TOEFL", "toefl", "70-120", "条件必填", "学生输入", "英语硬门槛、标化评分", "国际生英语要求缺官方数据时按推断规则处理"],
  ["语言", "IELTS", "ielts", "5.5-9.0", "可选", "学生输入", "英语硬门槛、标化评分", "全局推断英语底线为 IELTS 6.5；官方学校规则优先"],
  ...
  ["高中背景", "中国高中学校", "highSchoolID", "AdmitRanking 种子名单/其他", "必填", "AdmitRanking 参考 + 用户选择", "高中背景校准", "默认使用其他/手动评估学校；只有用户明确选择时才使用命名高中代理"],
  ["专业材料", "艺术作品集已准备", "hasPortfolio", "是/否", "艺术类必填", "学生输入", "作品集硬门槛", "艺术类未准备作品集时按推断规则归零"],
  ["选校", "计划选择大学数量", "requestedSchoolCount", "0+", "必填", "学生输入", "推荐组合", "点击自动推荐按钮后按总数生成学校；手动选择无人工上限"],
  ["选校", "自动推荐按钮", "autoRecommend", "按钮", "可选", "用户操作", "推荐组合", "不会在空选校时隐式推荐；点击后写入 selectedCollegeIDs"],
  ["选校", "手动目标学校", "selectedCollegeIDs", "已审核 v1 学校 ID 集合", "可选", "用户选择", "逐校概率、组合概率", "通过大学目录弹窗选择；手动选择优先于自动推荐"]
];
...
const probabilityRows = [
  ...
  ["画像总分", "profileScore", "学术、排名、课程、标化、活动等加权；test-free/test-blind 学校按校级政策排除 SAT/ACT；SAT/ACT 同时存在时取最强 SAT 等效分；testOptional=true 时忽略残留 SAT/ACT", "0-100", "调整普通申请池先验", "学生输入 + 学校政策", "当前实现使用透明固定权重，单校概率使用学校感知画像分"],
  ...
  ["组合警告", "selectionWarnings", "无效学校 ID 或组合层面的数据范围问题", "列表", "结果展示、报告", "计算引擎", "不在已审核 v1 数据集内的学校必须排除并披露"],
  ...
  ["顶尖高中强队列", "topCohortDelta", "一流国际学校 top20% 内按排名分段触发；T11-T30 修正高于 T10", "logit 修正 + 中国容量上限抬升", "强队列校准", "高中代理 + 校内排名", "解决一流国际学校 top10% 的非 T10 T30 组合被普通中国申请池过度压低的问题；不放开 T10 上限"],
  ["专业竞争修正", "majorDelta", "CS/Engineering/Business 等专业竞争", "logit 修正", "按专业调节", "模型规则", "CS/工程更保守"],
  ["轮次修正", "roundDelta", "ED/EA/RD", "logit 修正", "申请策略调节", "学生输入 + 学校规则", "缺少学校级明确优势数据时不泛化加分；允许/禁止轮次仍由 gate 管"],
  ["资助修正", "aidDelta", "是否申请资助", "logit 修正", "单独披露 need-aware / limited 风险", "学生输入 + 国际生资助政策", "仅适用于申请资助的国际生；need-blind 和美籍/绿卡申请者不扣分"],
  ["数据质量修正", "uncertaintyPenalty", "学校统计缺口 + gate 推断数量", "logit 负向修正", "置信度和概率保守化", "数据源完整性", "缺官方数据不得假装确定"],
  ["单校概率", "adjustedProbability", "logistic(logit(ordinaryPrior)+各项修正)，再按当前轮次中国录取容量封顶", "0%-82%，gate失败为0%", "逐校结果", "计算引擎", "展示 bucket 和 warnings"],
  ["推荐排名价值分", "recommendationRankScore", "同类学校内按排名线性映射，排名越靠前分数越高", "40-100", "自动推荐", "计算引擎", "避免只按录取概率推荐低价值学校"],
  ["推荐期望值", "recommendationExpectedValue", "单校录取概率 × 推荐排名价值分", "非负数", "自动推荐", "计算引擎", "组合选择时再叠加同层相关性边际折扣"],
  ["结果分档", "bucket", "0% 硬门槛未满足；>0 且 <20% 争取；20%-60% 目标；>=60% 保底", "中文标签", "结果展示、自动推荐", "计算引擎", "分档不是录取结论；保底也不是保证；自动推荐按用户指定总数生成并保持组合结构可解释"],
  ["组合来源", "selectionSource", "尚未选择 / 手动选择 / 自动推荐", "中文标签", "结果展示、报告", "App 状态 + 计算引擎", "推荐缺口只在自动推荐组合中展示"],
  ["组合结构", "selectedBucketCounts", "当前组合内保底/目标/争取/硬门槛未满足数量", "计数", "结果展示、报告", "计算引擎", "用于检查列表是否平衡"],
  ["推荐缺口", "recommendationWarnings", "自动推荐无法满足请求总数时生成提示", "列表", "结果展示、报告", "计算引擎", "总体可推荐学校不足时必须披露"],
  ["综大T10至少一所", "t10AtLeastOne", "当前组合内综合大学 T10 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "只统计用户手选或自动推荐组合"],
  ["综大T11-T30至少一所", "t11T30AtLeastOne", "当前组合内综合大学排名 11-30 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "对应用户口语中的“非 T10 的 T30”；与传统含 T10 的 T30 分开展示"],
  ["综大T30至少一所", "t30AtLeastOne", "当前组合内综合大学 T30 学校结果做相关性折扣组合；非 T10 T30 使用较轻折扣", "0%-98%", "汇总概率", "计算引擎", "不是全库 T30，也不是简单独立相乘；15 所 T11-T30 对一流国际学校 top10% 应接近/超过 90%"],
  ["综大T50至少一所", "t50AtLeastOne", "当前组合内综合大学 T50 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "当前组合外学校不参与"],
  ["文理T10至少一所", "liberalArtsT10AtLeastOne", "当前组合内文理学院 T10 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "只统计已选 Top30 文理学院内的 T10"],
  ["文理T30至少一所", "liberalArtsT30AtLeastOne", "当前组合内文理学院 T30 学校结果做相关性折扣组合", "0%-98%", "汇总概率", "计算引擎", "只统计已选 Top30 文理学院"],
  ["当前组合至少一所", "selectedAtLeastOne", "用户手选或自动推荐组合，合并综合大学与文理学院", "0%-98%", "组合结果", "计算引擎", "展示在结果页顶部"],
...
const sourceRows = [
  ["AdmissionSight College Acceptance Rates", "https://admissionsight.com/college-acceptance-rates/", "学校、US News Rank、Class 2029-2024 录取率", "综合大学统计主表", "v1 综合大学统计种子", "Class 2029 N/A 时用最近非空年份", "已内置 National Universities 种子数据"],
  ["Reviewed Top30 Liberal Arts Colleges table", "https://github.com/qyf9794/Admission-Calculator/blob/main/data/liberal_arts_colleges.csv", "Top30 文理学院、排名、整体录取率", "文理学院统计主表", "v1 文理学院统计种子", "用户提供 IMG_0742.JPG 表格抽取并人工审阅", "国际生率无分母，不作为 international_admit_coefficient"],
...
const methodRows = [
  ...
  ["7", "修正项叠加", "学校感知画像、高中、专业、轮次、资助、学术匹配、数据缺口、中国趋势", "adjustedLogit = logitPrior + deltas", "个性化概率", "所有 deltas 必须可解释；资助需求单独披露，不和轮次混在一个因子里"],
  ...
  ["10", "自动推荐组合", "用户点击按钮后按计划选择大学数量抽取学校", "requestedSchoolCount + likely/target/reach bucket labels", "写入 selectedCollegeIDs", "空选校不再隐式推荐；总体数量不足时披露；手动组合不显示自动推荐缺口结论"],
