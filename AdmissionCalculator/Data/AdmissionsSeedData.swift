import Foundation

enum AdmissionsSeedData {
    static let dataVersion = AdmissionsNormalizedData.dataVersion
    static let generatedAt = AdmissionsNormalizedData.generatedAt
    static let admissionsSightURL = AdmissionsNormalizedData.admissionsSightURL
    static let sourceRecords = AdmissionsNormalizedData.sourceRecords
    static let colleges = AdmissionsNormalizedData.colleges
    static let internationalSignals = AdmissionsNormalizedData.internationalSignals
    static let chinaAdmissionSignals = AdmissionsNormalizedData.chinaAdmissionSignals
    static let academicBenchmarks = AdmissionsNormalizedData.academicBenchmarks
    static let highSchools = AdmissionsNormalizedData.highSchools
    static let gateRules = AdmissionsNormalizedData.gateRules
}
