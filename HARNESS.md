...
- Clearly mark inferred gate rules in the data/model explanation sheet; do not use confidence or source completeness to change school probabilities.
- Only applicable gate rules may be checked or disclosed in the data/model explanation sheet; irrelevant global rules must not penalize unrelated applicants.
...
- The main app result surface should show probability results and user-needed missing inputs only. Confidence labels, system data gaps, and per-school source audit notes must be available from a separate data/model explanation link or sheet, not inline in the main result content.
- Per-school source audit in the explanation sheet must disclose structured round policy fields, including allowed rounds and any explicit EA/ED probability adjustments.
...
- AI reports for automatic portfolios must explain that recommendation uses probability × rank-value score with same-tier marginal discounting and expected best-admit value, not raw probability sorting.
- AI reports must include each selected school's probability in the report page, but should not surface confidence labels, source-audit summaries, or system missing-data explanations in the report body.
...
- AI reports should focus on user-useful planning: per-school probability, application-count impact, major probability drivers, student-vs-school benchmark gaps/advantages, improvement actions, and portfolio strategy.
...
- Explicit EA/ED probability adjustments are bounded observational calibrations, not official causal guarantees; binding ED/ED2 may receive a stronger school-specific adjustment than non-binding EA, while official no-advantage policies must keep a zero adjustment.
...
8. Display probability results and user-needed missing inputs in the main flow; keep confidence, system data gaps, and source audit details behind a separate explanation link/sheet.
...
why a probability is low or high, while keeping technical source and confidence details available outside the main flow.
